...
class metricor:
    ...
    def process_threshold(self, args):
        labels, score, P, L, threshold = args
    ...
    def RangeAUC_faster(self, labels, score, window=0, percentage=0, plot_ROC=False, AUC_type='window'):
        ...
        th_vals = np.linspace(0, len(score) - 1, 250).astype(int)

        args = [(labels, score, P, L, score_sorted[th_val_i]) for th_val_i in th_vals]

        with Pool(cpu_count()) as pool:
            results = pool.map(process_threshold, args)

        # Unpack the results
        TPR_list, FPR_list, Precision_list = zip(*results)

        TPR_list = list(TPR_list)
        FPR_list = list(FPR_list)
        Precision_list = list(Precision_list)
        TPR_list.insert(0, 0)
        FPR_list.insert(0, 0)

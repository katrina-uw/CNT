...
class MSL(Dataset):
    ...
    def __init__(self, seed: int, verbose=False, normalize=False, normalize_type="minmax"):
        ...
        self.root = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                                           "data", "smap_msl")
        self.processed_path_train = os.path.join(self.root, "processed", "msl_train.pkl")
        self.processed_path_test = os.path.join(self.root, "processed", "msl_test.pkl")
        self.processed_path_test_label = os.path.join(self.root, "processed", "msl_test_label.pkl")
    ...
    def load(self):
        ...
        with open(self.processed_path_train,"rb") as f:
            train_data = pickle.load(f).reshape((-1, 55))

        with open(self.processed_path_test, "rb") as f:
            test_data = pickle.load(f).reshape((-1, 55))
    ...
    def get_start_position(self, is_train):
        if is_train:
            md = pd.read_csv(os.path.join(self.root, 'msl_train_md.csv'))
        else:
            md = pd.read_csv(os.path.join(self.root, 'labeled_anomalies.csv'))
            md = md[md['spacecraft'] == "MSL"]
